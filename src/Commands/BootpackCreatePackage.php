...
class BootpackCreatePackage extends Command
{
     ...
     * @var string
     */
    protected $signature = 'zng:add {name : Package name} {--path : Location to create the package}';
    ...
    public function handle()
    {
        ...
        if ($this->confirm('The package creation is going to start, type yes to begin', 'yes')) {
            if (!is_dir($path)) {
                ...
                rename(
                    $path . '/src/Services/Services.php',
                    $path . '/src/Services/' . ucfirst($p_name) . 'Services.php'
                );
                rename(
                    $path . '/src/Model/Model.php',
                    $path . '/src/Model/' . ucfirst($p_name) . 'Model.php'
                );
